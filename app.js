...
let prompts = [
  {
    name: 'v1-技术岗初筛',
    content:
      '你是招聘助手。请根据JD与简历内容，从技能匹配、项目经验、稳定性三个维度评分。输出JSON：{"score": number, "reasons": string[] }',
  },
  {
    name: 'v2-销售岗初筛',
    content:
      '你是招聘助手。重点评估销售业绩、行业资源、沟通能力。输出JSON：{"score": number, "reasons": string[] }',
  },
  {
    name: 'v1-JD清洗通用',
    content:
      '你是JD优化助手。请保留核心要求，去除歧义和冗余，输出结构：岗位职责、任职要求、加分项。',
  },
];
...
function getPromptContent(name) {
  return prompts.find((p) => p.name === name)?.content || '';
}
...
function resetForm() {
  ...
  ui.promptContent.value = getPromptContent(ui.promptSelect.value);
...
window.onEdit = function onEdit(id) {
  ...
  ui.promptContent.value = getPromptContent(s.prompt);
...
ui.promptSelect.addEventListener('change', () => {
  ui.promptContent.value = getPromptContent(ui.promptSelect.value);
});
ui.savePromptBtn.addEventListener('click', () => {
  const selected = ui.promptSelect.value;
  const content = ui.promptContent.value.trim();
  const target = prompts.find((p) => p.name === selected);
  if (!target) {
    ui.result.textContent = '保存失败：未找到对应提示词模板。';
    return;
  }
  target.content = content;
  ui.result.textContent = `提示词模板已更新：${selected}`;
});
